...
def process_single_pdf(pdf_info):
    ...
    if markdown_content:
        ...
        try:
            ...
            return f"✓ {filename} → {md_filename}"
        except Exception as e:
            return f"✗ Fehler beim Speichern {filename}: {e}"
    else:
        return f"✗ Keine Inhalte extrahiert aus {filename}"


def main():
    """Hauptfunktion – keine Hardcodierung, läuft im aktuellen Ordner"""
    current_dir = Path.cwd()
    input_dir = current_dir / "data/raw"
    ...
    print(f"\n📄 Gefunden: {len(pdf_files)} PDF-Dateien")

    if not pdf_files:
        return

    print(f"⚙️ Starte Verarbeitung mit max. 4 Threads...\n")
